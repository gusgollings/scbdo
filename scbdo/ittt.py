...
individual track time trial events:
...
class ittt(object):
    ...
    def toarmstart(self):
        ...
        if doarm:
            ...
            self.showtimerwin()
            ## TODO: Print linefeed and event summary on receipt
            self.meet.printimp(True)
            if self.fs.status == 'armstart':
                self.meet.scbwin.sett1('       0.0     ')
                ## TODO: Print rider name on receipt with no.
            if self.bs.status == 'armstart':
    ...
    def __init__(self, meet, event, ui=True):
        ...
        self.log = logging.getLogger('scbdo.ittt')
        self.log.setLevel(logging.DEBUG)
        self.log.debug('Creating new ittt event: ' + str(self.evno))
	...
	self.splits = []		# list of lap split lists
        ...
        b = gtk.Builder()
        b.add_from_file(os.path.join(scbdo.UI_PATH, 'ittt.ui'))
